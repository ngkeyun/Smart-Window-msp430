...
int targetServoPos = openPos;  // What position we WANT to be at

// SERVO SPEED CONTROL
unsigned long lastServoMoveTime = 0;
// Your original code assumes a 15,000ms (15s) move for a 1000-step
// range (2000-1000), which is 15ms per step.
// For half speed, we will use a 30ms delay for each 1-microsecond step.
...
void autoLogic() {
  ...
  windowOpen = !isRaining; // true = open, false = closed

  // Set the "isClosing" flag ONLY if the window is NOT
  // in the closed position, but its target IS the closed position.
  // This flag is used for the display and safety check.
  if (windowOpen == false && currentServoPos != closePos) {
    isClosing = true;
  } else {
    isClosing = false;
  }
  
  // No timer needed! The "isClosing" state will automatically
  // turn off when the servo physically reaches 'closePos'.
}
 ...
 **************************************************************/
/**************************************************************
 * SERVO CONTROL – MODIFIED FOR RAMPING
 **************************************************************/
void applyServoMovement() {

  // 1. Determine the absolute target position based on the desired state
  if (windowOpen) { // This is set by autoLogic
    targetServoPos = openPos;
  } else {
    targetServoPos = closePos;
  }

  // 2. Check for safety overrides
  if (systemState == EMERGENCY_ACTIVE) {
    // Emergency stop: Stop where we are.
    // Set the target to our current position so we don't move.
    targetServoPos = currentServoPos; 
    return; // No command sent, failsafe will activate
  }

  if (isClosing && blocked) {
    // Obstacle: Stop where we are.
    targetServoPos = currentServoPos;
    return; // No command sent, failsafe will activate
  }

  // 3. Check if we are already at the target
  if (currentServoPos == targetServoPos) {
    return; // Already there, do nothing (failsafe will activate)
  }

  // 4. Check if it's time to move one step
  if (millis() - lastServoMoveTime < SERVO_STEP_DELAY_MS) {
    // Not time to move yet, exit
    return;
  }

  // --- If we are here, it's time to move one step ---
  
  // 5. Update the timer
  lastServoMoveTime = millis();

  // 6. Move one step closer to the target
  if (currentServoPos < targetServoPos) {
    currentServoPos++;
  } else {
    currentServoPos--;
  ...
  // 7. Send the new, single-step position to the servo
