 ...
 * Features:
 ...
 - SAFETY (Option B): Distance sensor only blocks while closing
 - AUTO MODE TOGGLE: TinyScreen Upper-Right button
       - Auto ON  -> system controls window
       - Auto OFF -> servo stopped, you can manually push window
 - EMERGENCY STOP: Wireling button on Port 0 (toggle)
 - Flashing display during emergency
 - Simple text UI on TinyScreen+
 */
...
//  Variables
bool emergencyStop = false;   // true = emergency active
...
void loop() {
  ...
  if (buttonValue < 100) {   // pressed
    emergencyStop = !emergencyStop;   // toggle
    if (emergencyStop) {
      servo.setServo(servoChannel, stopPos); // hard stop
    }
    delay(300); // debounce
  }
...
void autoLogic() {
  ...
  // If currently closing and obstacle is detected:
  if (isClosing && blocked) {
    // Do not cancel isClosing flag,
    // but servo will be held at STOP in applyServoMovement()
    // until obstacle is removed.
  }
...
void applyServoMovement() {
  if (isClosing) {
    // Only drive servo if NOT blocked.
    if (!blocked) {
      servo.setServo(servoChannel, closePos);
    } else {
      servo.setServo(servoChannel, stopPos); // stop due to obstacle
    }
  }
  else if (windowOpen) {
...
void drawNormalDisplay() {
  ...
  display.print("Mode: ");
  display.print("AUTO");   // because manualMode == false in this function

  // Optional: show if blocked
  if (blocked) {
...
void drawManualDisplay() {
  ...
  // Note for you (logic explanation):
  // In this mode, servo is sent STOP (1500us) so it does not drive.
  // The user can manually push the physical window open/close.
}
