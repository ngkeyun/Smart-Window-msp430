...
void handleButtonPress() {
  ...
  // Prevent multiple toggles from same press
  // Only act if enough time has passed since the previous toggle
  if (millis() - lastButtonPress < BUTTON_DEBOUNCE) return;
  lastButtonPress = millis();

  // ----- BEHAVIOR -----
  // If we are in emergency or blocked → pressing button should EXIT EMERGENCY
  if (systemState == EMERGENCY_ACTIVE || systemState == OBSTACLE_BLOCKED) {
    systemState = NORMAL;
    isClosing = false;
    return;
  }

  // If we are normal → pressing button ENTERS EMERGENCY
  systemState = EMERGENCY_ACTIVE;
  isClosing = false;
}
...
void applyServoMovement() {
  ...
  // Emergency stop or obstacle
  if (systemState == EMERGENCY_ACTIVE) return;
  if (systemState == OBSTACLE_BLOCKED) return;

  // 1. CLOSING (rain detected)
  if (isClosing) {
    slowMoveServo(currentServoPos, closePos, 30, 500); 
    lastWindowOpen = false;
    return;
  }

  // 2. OPENING (rain stopped)
  if (windowOpen && !lastWindowOpen) {
    slowMoveServo(currentServoPos, openPos, 30, 500);
    lastWindowOpen = true;
    return;
  }

  // 3. Idle → do nothing, stop sending servo signals
...
// void slowMoveServo(int startPos, int endPos, int steps, int delayTime) {
//   int stepSize = (endPos - startPos) / steps;

//   for (int i = 0; i < steps; i++) {

//     // 1) Check emergency button BEFORE moving this step
//     Wireling.selectPort(PORT_BUTTON);
//     int buttonValue = analogRead(A0);
//     if (buttonValue < 100) {
//       // Enter emergency state and remember this press time
//       systemState = EMERGENCY_ACTIVE;
//       lastButtonPress = millis();  // prevent instant unfreeze from same press
//       return;
//     }

//     // 2) If already in EMERGENCY or OBSTACLE state, abort movement
//     if (systemState == EMERGENCY_ACTIVE) return;
//     if (systemState == OBSTACLE_BLOCKED) return;

//     // 3) Move one small step
//     int newPos = startPos + (stepSize * i);
//     servo.setServo(servoChannel, newPos);
//     currentServoPos = newPos;

//     // 4) Instead of one big delay, wait in small chunks and
//     //    keep checking the emergency button during the wait
//     unsigned long startWait = millis();
//     while (millis() - startWait < (unsigned long)delayTime) {
//       Wireling.selectPort(PORT_BUTTON);
//       buttonValue = analogRead(A0);

//       // If emergency pressed during the wait, trigger emergency and exit
//       if (buttonValue < 100) {
//         systemState = EMERGENCY_ACTIVE;
//         lastButtonPress = millis();  // debounce against handleButtonPress()
//         return;
//       }

//       // Also honour any state change from outside
//       if (systemState == EMERGENCY_ACTIVE) return;
//       if (systemState == OBSTACLE_BLOCKED) return;

//       delay(5);  // tiny pause, so we’re not busy-looping
//     }
//   }

//   // Final exact position
//   servo.setServo(servoChannel, endPos);
...
void slowMoveServo(int startPos, int endPos, int steps, int delayTime) {
  ...
  for (int i = 0; i < steps; i++) {
    ...
    //------------------------------
    // 4. Fine-delay loop (5ms checks)
    //------------------------------
    unsigned long startWait = millis();
    while (millis() - startWait < (unsigned long)delayTime) {

      // EMERGENCY pressed during wait
      Wireling.selectPort(PORT_BUTTON);
      buttonValue = analogRead(A0);
      if (buttonValue < 100) {
        systemState = EMERGENCY_ACTIVE;
      ...
      //----------------------------------------
      //  DISTANCE SENSOR — LIVE OBSTACLE CHECK
      //----------------------------------------
      Wireling.selectPort(PORT_DISTANCE);
      int mm = distanceSensor.readRangeContinuousMillimeters();
      ...
      if (mm > 0 && mm < 100) {
        ...
        blocked = false;  // clear when no obstacle
      }

      // Respect outside state changes
      if (systemState == EMERGENCY_ACTIVE) return;
      if (systemState == OBSTACLE_BLOCKED) return;
